...
int main(int argc, char* argv[]) {
    ...
    int sort_mode = 0;        // 0: 按时间排序（默认），1: 按大小升序(-min)，2: 按大小降序(-max)
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "-h") {
            std::cout << "Usage: " << argv[0] << " [OPTIONS] [directory]\n"
                      << "\nOptions:\n"
                      << "  -l <number>     Limit output to N directories (default: no limit)\n"
                      << "  -print0         Use null character as delimiter (for use with xargs -0)\n"
    ...
    if (sort_mode == 1) {
        std::ranges::stable_sort(all_directories.begin(), all_directories.begin() + max_output,
            [](const DirInfo& a, const DirInfo& b) {
